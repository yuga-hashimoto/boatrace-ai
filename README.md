...
python -m boatrace_ai report-live --race-date 2026-03-19 --lookahead-minutes 90 --state-path artifacts/reports/live_report_state.json
...
`report-live` is the chat-facing wrapper for automation. It runs the same live prediction flow with recommendations, prints a Discord-friendly summary of current bets or top picks, and appends newly settled hit/miss outcomes from earlier live recommendations while suppressing duplicates via a state file.
